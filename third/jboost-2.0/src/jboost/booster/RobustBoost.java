...
public class RobustBoost extends AbstractBooster {
  ...
  public void update(Prediction[] predictions, int[][] exampleIndex) {
    ...
    for (int i = 0; i < predictions.length; i++) {
      ...
      // if this is a zero prediction, we skip
      ...
      // how many iterations in NewtonSolver before we give up
      int max_iter = 30;

      boolean foundSolution = false;
      double ds = Double.NaN, dt = Double.NaN;

      // create a solver
      NewtonSolver ns = new NewtonSolver(this, mask, value, max_iter);
      ...
      if (ns.canFinishNow()) {
        ...
        // if failed, do more search with starting point on a grid
        if (!foundSolution) {
          done: for (double temp_ds = DS_MIN; temp_ds < DS_MAX; temp_ds += DS_STEP) {
            for (double temp_dt = DT_MIN; temp_dt <= 1 - m_t; temp_dt += DT_STEP) {
              ns.solve(temp_ds, temp_dt);
              if (ns.isSucceeded()) {
                ds = ns.getDs();
                dt = ns.getDt();
                foundSolution = true;
      ...
      if (foundSolution && ds != Double.NaN && dt != Double.NaN && dt > 0) {
      ...
      // no solutions found
        ...
        System.out.println("Warning: Newton's method has failed. This might or might not be a problem.");
  ...
  public class RobustBinaryBag extends Bag {
    ...
    public BinaryPrediction calcPrediction() {
      ...
      if (Double.isNaN(totalWeight) || Math.abs(totalWeight) < EPS || Math.abs(m_w[0] - m_w[1]) < EPS) {
        ...
        // normalized linear scale [-1, 1]
        double norm_linear = (m_w[1] - m_w[0]) / totalWeight;
        return new BinaryPrediction(norm_linear);
      }
    }
  ...
  public class NewtonSolver {
    ...
    public NewtonSolver(RobustBoost rb, boolean[] mask, double[] value, int max_iter) {
    ...
    public void solve(double init_ds, double init_dt) {
      ...
      for (int i = 0; i < max_iter; i++) {
        ...
        // Found a solution
        if (Math.abs(F[0]) < RHS_EPS && Math.abs(F[1]) < RHS_EPS && dt > 0) {
